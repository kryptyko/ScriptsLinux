...
mount_nas() {
    echo "Instalando NAS..."
    sudo mkdir /mnt/nas
    echo "//ipdelservidor/datos /mnt/nas cifs rw,uid=administrador,gid=administrador,vers=1.0,user=usuarioremoto,domain=dominio,password=contraseña" >> "/etc/fstab"
    sudo mount -a
}
...
unmount_nas() {
    ...
    sed -i '/\/\/192\.168\.1\.10\/datos/d' "/etc/fstab"
}
